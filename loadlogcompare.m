...
%     countdowns = sort(countdowns);
    ...
    allDone = 0;
    endToParse = 0;
    %for each allocated countdown:
    while allDone < numel(countdowns)
        successRateStorage = 0;
        beginToParseAt = endToParse+1;
        disp(['beginToParseAt = ',num2str(beginToParseAt)])
        succeedingIndexes = [];
        qualityStorage = [];
        idx = 1;
        %as long as the datas are related to that countdown:
        while (idx+1<=numel(allCells{i,1})) && (allCells{i,1}{idx+1,1}.countdown == allCells{i,1}{idx,1}.countdown)
            disp('(idx+1<=numel(allCells{i,1})) && (allCells{i,1}{idx+1,1}.countdown == allCells{i,1}{idx,1}.countdown)')
            successRate = allCells{i,1}{idx,1}.success;
            ...
            if successRate ~= 0 %get the j<->query+run combination for which a solution exists
                disp('ok')
                succeedingIndexes(end+1) = beginToParseAt+endToParse; %not mandatory
                qualityStorage(end+1) = allCells{i,1}{succeedingIndexes(end),1} %.seqM %(end);
            end
            
            endToParse = endToParse+1;
            idx = beginToParseAt+endToParse;
            disp(['endToParse = ',num2str(endToParse)])
            disp(['idx = ',num2str(idx)])
        end
        %TODO: deal with the last block
        
        successRates(end+1) = successRateStorage/(endToParse-beginToParseAt+1);
        if numel(qualityStorage) ~= 0
            qualities(end+1) = sum(qualityStorage)/numel(qualityStorage);
        else
            qualities(end+1) = 0;
        end
    ...
    stru.successRates = successRates;
    stru.qualities = qualities;
    allPlanners(end+1) = stru;
end